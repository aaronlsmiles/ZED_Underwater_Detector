...
def save_screenshot(image, timestamp):
...
def torch_thread(weights, img_size, conf_thres=0.2, iou_thres=0.45):
    ...
    while not exit_signal:
        if run_signal:  # Remove pause_signal check here to keep thread running
            lock.acquire()
            ...
            # Only process and save detections if not paused
            if not pause_signal and len(det) > 0:
                ...
                for d in det:
                    ...
                    distance = depth.get_value(center_x, center_y)[1]
            ...
            detections = detections_to_custom_box(det, image_net) if not pause_signal else []
            lock.release()
            run_signal = False
        sleep(0.01)
...
def main():
    global image_net, exit_signal, run_signal, detections, class_names, zed, pause_signal, detection_data, excel_filename
    # Initialize Excel file at program start (add after camera initialization)
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            current_image = image_left_tmp.get_data()  # Store current image
            if pause_signal:
                # Create black image with same dimensions as camera feed
            ...
            # 2D rendering
            np.copyto(image_left_ocv, image_left.get_data() if not pause_signal else np.zeros_like(image_left.get_data()))
            ...
            if key == 27:  # ESC key
                ...
                if pause_signal:
                    ...
                    # Save screenshot before blacking out the display
                    save_screenshot(current_image, timestamp)

                    # Add empty row to DataFrame to mark pause
                    detection_data = pd.concat([detection_data, pd.DataFrame(
                        {'Class': [''], 'Name': [''], 'Confidence': [''],
                         'Distance': [''], 'Timestamp': [f'---PAUSED--- (screenshot_{timestamp}.png)']})],
                        ignore_index=True)
                    save_to_excel()  # Save data when pausing
                else:
                    print("\n--- Detection Resumed ---")
        elif zed.grab(runtime_params) != sl.ERROR_CODE.SUCCESS:  # Only exit on grab error
            exit_signal = True
            save_to_excel()  # Save data when exiting
