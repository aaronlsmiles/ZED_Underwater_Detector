            ...
            if not pause_signal:
                ...
                if len(detections) > 0:
                    ...
                    for i, obj in enumerate(objects.object_list):
                        ...
                        center_distance = point3D[2] / 1000.0 if err == sl.ERROR_CODE.SUCCESS else 0.0

                        # Get object measurements
                        if obj.tracking_state == sl.OBJECT_TRACKING_STATE.OK:
                            object_distance = float(obj.position[2]) / 1000.0  # Convert to meters

                            # Get 3D bounding box
                            bbox3d = obj.bounding_box
                            if bbox3d is not None and len(bbox3d) >= 8:  # Check if we have all 8 corners
                                try:
                                    # Calculate dimensions based on the correct corner indices
                                    # Front face: 0,3,4,7
                                    # Back face: 1,2,5,6
                                    width = abs(float(bbox3d[3][0] - bbox3d[0][0])) / 1000.0   # x-axis (left to right)
                                    height = abs(float(bbox3d[4][1] - bbox3d[0][1])) / 1000.0  # y-axis (top to bottom)
                                    depth = abs(float(bbox3d[1][2] - bbox3d[0][2])) / 1000.0   # z-axis (front to back)
                        ...
                        print(f"Class: {class_id}, Name: {class_names[class_id]}, Confidence: {confidence:.2%}, "
                              f"Center Distance: {center_distance:.4f}m, Object Distance: {object_distance}m\n"
                              f"Dimensions (WxHxD): {width}m x {height}m x {depth}m")
