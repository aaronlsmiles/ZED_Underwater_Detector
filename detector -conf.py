    ...
    while not exit_signal:
        if run_signal:
            ...
            for d in det:
                ...
                print(f"Class: {class_id}, Name: {model.names[class_id]}, Confidence: {conf:.2%}")
...
def main():
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            # Print confidence scores for detected objects
            if objects.is_new:
                obj_array = objects.object_list
                print("\n---- New Detection ----")
                for obj in obj_array:
                    class_name = class_names[obj.label] if hasattr(class_names, '__getitem__') and obj.label in class_names else "Unknown"
                    print(f"Label '{class_name}' (ID: {obj.label}) with confidence: {int(obj.confidence)}%")
                print("-----------------------\n")
